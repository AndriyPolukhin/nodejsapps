...
/*
// 4.8 LIST LOGS (USING BULT-IN FUNCTION)
cli.responders.listLogs = () => {
...
*/
// 4.8-ALT LIST LOGS (USING SHELL)
cli.responders.listLogs = () => {
  // 4.8.1 INIT THE CHILD PROCESS
  const ls = childProcess.spawn('ls', ['./.logs/']);
  ls.stdout.on('data', (dataObject) => {
    // 4.8.2 Explode into separate lines
    const dataStr = dataObject.toString();
    const logFileNames = dataStr.split('\n');

    // 4.8.3 Loop through, so for each file name print it's name
    cli.verticalSpace();
    logFileNames.forEach((logFileName) => {
