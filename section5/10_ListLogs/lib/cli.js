...
cli.responders.moreUserInfo = (str) => {
  ...
  if (userId) {
    ...
    _data.read('users', userId, (err, userData) => {
      if (!err && userData) {
        // 4.5.3 Remove the hashed password
...
cli.responders.listChecks = (str) => {
  ...
  _data.list('checks', (err, checkIds) => {
    if (!err && checkIds && checkIds.length > 0) {
      cli.verticalSpace();
      // 4.6.2. Loop throught for the individual check
      checkIds.forEach((checkId) => {
        _data.read('checks', checkId, (err, checkData) => {
          ...
          // 4.6.6. If theres a state, or hasn't specified any state include the current check accourdingly
          if (
...
cli.responders.moreCheckInfo = (str) => {
  ...
  // 4.7.1 Get the id from the stirng that was provided
  let arr = str.split('--');
...
cli.responders.listLogs = () => {
  ...
  // 4.8.1 List All logs with a built-previously function form the _logs lib
  _logs.list(true, (err, logFileNames) => {
    if (!err && logFileNames && logFileNames.length > 0) {
      cli.verticalSpace();
      // 4.8.2 Loop throught, so for each of the filenames we print its name
