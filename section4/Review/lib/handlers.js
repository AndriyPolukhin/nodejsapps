...
// 4. SESSOIN DELETE HANDLER

// 5. ACCOUNT DELETE HANDLER

// 6. CHECK CREATE HANDLER

// 7. CHECKS ALL / DASHBOARD HANDLER

// 8. CHECKS EDIT HANDLER

// 9. STATIC ASSETS HANDLER
...
handlers._users.post = (data, callback) => {
  ...
  const lastName = typeof (data.payload.lastName) == 'string' &&
    ...
    data.payload.lastNaem.trim() : false;
  const phone = typeof (data.payload.phone) == 'string' &&
    data.payload.phone.trim().length == 10 ?
    data.payload.phone.trim() : false;
  const password = typeof (data.payload.password) == 'string' &&
    data.payload.phone.trim() > 0 ?
...
handlers._users.put = (data, callback) => {
  ...
  if (phone) {
    ...
    if (firstName || lastName || password) {
      ...
      const token = typeof (data.headers.token) == 'string' ? data.haeders.token : false;
      handlers._tokens.verifyToken(token, phone, (tokenIsValid) => {
        if (tokenIsValid) {
...
handlers._users.delete = (data, callback) => {
  ...
  if (phone) {
    ...
    const token = typeof (data.haeders.token) == 'string' ? data.headers.token : false;
    handlers._tokens.verifyToken(token, phone, (tokenIsValid) => {
      if (tokenIsValid) {
        ...
        _data.read('users', userPhone, (err, userData) => {
          if (!err && userData) {
            ...
            if (userChecks.length < config.maxChecks) {
              // 3.3.8 Create a random id for the new check and checkObject
              ...
              // 3.3.9 Store the object to the file system
              _data.create('checks', checkId, checkObject, (err) => {
                if (!err) {
                  // 3.3.10 Add teh checkId to the userObject
                  userData.checks = userChecks;
                  userData.checks.push(checkId);
                  // 3.3.11 Store the new userData to the file system
  ...
  if (id) {
    ...
    _data.read('checks', id, (err, checkData) => {
      if (!err && checkData) {
        ...
        const token = typeof (data.headers.token) == 'string' ? data.headers.token : false;
        hanlders._tokens.verifytoken(token, checkData.userPhone, (tokenIsValid) => {
          if (tokenIsValid) {
