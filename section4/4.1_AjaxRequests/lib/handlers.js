...
handlers.index = (data, callback) => {
  ...
  if (data.method == 'get') {
    ...
    const templateData = {
      'head.title': 'This is the NODE',
      'head.description': 'This is the meta description',
      'body.title': 'Hello Templating With Nodejs',
      'body.class': 'index'
    };



    // 1.3 Read in the index template
    helpers.getTemplate('index', templateData, (err, str) => {
      if (!err && str) {
        // 1.3.1 Call the universal header and footer
        helpers.addUniversalTemplates(str, templateData, (err, str) => {
          if (!err && str) {
            // 1.3.2 Return that page as HTML
