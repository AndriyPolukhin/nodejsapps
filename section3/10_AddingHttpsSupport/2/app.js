...
const server = http.createServer((req, res) => {
  ...
  req.on('end', () => {
    ...
    // 2.6.1 Chose the handlers this request should go to
    ...
    choseHandler(data, function (statusCode, payload) {
      ...
      // 2.6.4-2 Use the payload provided by the router, or use the default object: {}
      payload = typeof (payload) == 'object' ? payload : {};
      // 2.6.4-3 Convert the payload into a string
      const payloadString = JSON.stringify(payload);
      // 2.6.4-4 Return the response: Content-Type, statusCode, payload
...
// 3. Listener
server.listen(config.port, () => console.log(`Server is running on port: ${config.port} under the ${config.envName} mode`));
