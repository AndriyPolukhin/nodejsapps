...
server.unifiedServer = (req, res) => {
  ...
  req.on('end', () => {
    ...
    try {
      ...
      // 5.7.5 THE ERROR IS THROWN INTO THE CONSOLE, app is still running
      debug(e);
...
// 6. PROCESS THE RESPONSE FROM THE HANDLER
server.processHandlerResponse = (res, method, trimmedPath, statusCode, payload, contentType) => {
  // 6.1 CONTENT TYPE: determine
  contentType = typeof (contentType) == 'string' ? contentType : 'json';
  // 6.2 STATUS CODE: determine
  statusCode = typeof (statusCode) == 'number' ? statusCode : 200;

  // 6.3 CONTENT SPECIFIC: parts
  let payloadString = '';
  if (contentType == 'json') {
  ...
  // 6.4 CONTENT COMMON: parts
  res.writeHead(statusCode);
  res.end(payloadString);

  // 6.5 Status code Colors
  if (statusCode == 200) {
